...
export default function RowComputers({ computer, index }) {
  const { handlerRemoveComputer } = useComputers();
  return (
    <tr key={index}>
      ...
      <td className="px-4 py-4 text-sm text-gray-300 flex flex-col">
        {computer.peripherals.map((peripheral, index) => (
          <span key={index}>{peripheral.name}</span>
        ))}
      </td>
      ...
      <td className="px-4 py-4 text-sm whitespace-nowrap">
        <div className="flex items-center gap-x-6">
          <NavLink
            to={`/inventory/computadoras/editar/${computer.id}`}
          ...
          <button
            ...
            onClick={() => handlerRemoveComputer(computer.id)}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
